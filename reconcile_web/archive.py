...
Docs: https://doyu.github.io/reconcile-web/archive.html.md"""
...
# %% ../nbs/00_archive.ipynb #733c9c20
def month_zip(
    archive_dir: str|Path,  # archive root
    month: str,             # month name like '2025-07'
) -> bytes:                 # zip of the whole month dir, paths relative to it
    "Bundle the month dir into an in-memory zip; raise FileNotFoundError on any escape"
    if not MONTH_RE.match(month): raise FileNotFoundError(month)
    root = Path(archive_dir).resolve()
    mdir = (root/month).resolve()
    if not (mdir.is_relative_to(root) and mdir.is_dir()): raise FileNotFoundError(month)
    buf = io.BytesIO()
    with zipfile.ZipFile(buf, 'w', zipfile.ZIP_DEFLATED) as zf:
        # every entry is containment-checked, not just files: rglob never descends into
        # dir symlinks, so an escaping one shows up only as this single non-file entry —
        # a files-only pass would drop it unchecked and its subtree would silently
        # vanish from the zip. Fail closed instead: never a partial zip.
        for p in sorted(mdir.rglob('*')):
            try: ok = p.resolve().is_relative_to(mdir)
            except (ValueError, RuntimeError, OSError):  # null bytes, symlink loops, ...
                raise FileNotFoundError(p) from None
            if not ok: raise FileNotFoundError(p)
            if p.is_file(): zf.write(p, arcname=p.relative_to(mdir))
    return buf.getvalue()
