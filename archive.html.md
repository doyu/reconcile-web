...
def make_archive(months=('2025-07', '2025-08')):
    ...
    for m in months:
        ...
        (d/'statement.html').write_text('<html>fake statement</html>')
...
assert safe_file(root, '2025-07', 'statement_nda').name == 'statement.nda'
assert safe_file(root, '2025-07', 'statement_html').name == 'statement.html'
...
*Bundle the month dir into an in-memory zip; raise FileNotFoundError on
any escape*
...
m = root/'2025-07'
(m/'reports').mkdir()
(m/'reports'/'2025-07_tase.pdf').write_bytes(b'%PDF-1.4 fake report')
z = zipfile.ZipFile(io.BytesIO(month_zip(root, '2025-07')))
# the zip mirrors the month dir: every file, relative sorted paths, subdirs preserved
expected = [str(p.relative_to(m)) for p in sorted(m.rglob('*')) if p.is_file()]
assert z.namelist() == expected
assert 'reports/2025-07_tase.pdf' in expected and 'receipts/2025-07-29_openai.pdf' in expected
assert z.read('statement.pdf') == (m/'statement.pdf').read_bytes()
...
# any escape refuses the WHOLE zip — fail closed, never a partial zip
m8 = root/'2025-08'
(m8/'receipts'/'evil.pdf').symlink_to('/etc/hostname')
assert zrejects(root, '2025-08')                    # escaping file symlink
(m8/'receipts'/'evil.pdf').unlink()
(m8/'evil_dir').symlink_to('/etc')
assert zrejects(root, '2025-08')                    # escaping dir symlink: must 404, not silently vanish
(m8/'evil_dir').unlink()
assert zipfile.ZipFile(io.BytesIO(month_zip(root, '2025-08'))).namelist()  # clean again -> works
assert zrejects(root, '../x')                       # traversal
assert zrejects(root, '2025-7')                     # not YYYY-MM
assert zrejects(root, '2031-01')                    # nonexistent month
outside = Path(tempfile.mkdtemp())
(outside/'statement.pdf').write_bytes(b'%PDF outside')
(root/'2031-12').symlink_to(outside)
assert zrejects(root, '2031-12')                    # month dir symlinked out of the archive
```
