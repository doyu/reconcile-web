...
def make_archive(months=('2025-07', '2025-08')):
    ...
    for m in months:
        ...
        (d/'statement.nda').write_text('T00fake\r\n')
        (d/'statement.html').write_bytes('<html>Työeläke</html>'.encode('latin-1'))  # bank HTML is ISO-8859-1
...
assert '/m/2025-07/statement.nda' in r.text and '/m/2025-07/statement.html' in r.text
assert client.get('/m/1999-01').status_code == 404            # unknown month

# months predating the bank-issued raw files get no dead links
(root/'2025-08'/'statement.nda').unlink()
(root/'2025-08'/'statement.html').unlink()
r = client.get('/m/2025-08')
assert '/m/2025-08/statement.nda' not in r.text and '/m/2025-08/statement.html' not in r.text
assert '/m/2025-08/statement.pdf' in r.text and '/m/2025-08/statement.csv' in r.text
```
...
assert client.get('/m/2025-07/statement.nda').text.startswith('T00')
h = client.get('/m/2025-07/statement.html')
assert h.status_code == 200 and h.headers['content-type'] == 'text/html; charset=ISO-8859-1'
assert 'Työeläke' in h.text   # the HTTP header overrides the file's <meta charset>; ä/ö must survive
(root/'2025-08'/'statement.nda').unlink()
assert client.get('/m/2025-08/statement.nda').status_code == 404   # month without the raw file
...
``` python
# month zip: authed one-download bundle of the whole month dir
import io, zipfile
root = make_archive()
(root/'2025-07'/'reports').mkdir()
(root/'2025-07'/'reports'/'2025-07_tase.pdf').write_bytes(b'%PDF-1.4 fake report')
app = create_app(archive_dir=root, password='pw', session_secret='s3cret')
client = TestClient(app, base_url='https://testserver')
client.post('/login', data={'password': 'pw'})

anon = TestClient(app, base_url='https://testserver')
assert anon.get('/m/2025-07/all.zip', follow_redirects=False).status_code == 303  # never bypasses auth
r = client.get('/m/2025-07/all.zip')
assert r.status_code == 200 and r.headers['content-type'] == 'application/zip'
assert r.headers['content-disposition'] == 'attachment; filename="ninjalabo-2025-07.zip"'
z = zipfile.ZipFile(io.BytesIO(r.content))
assert 'status.md' in z.namelist() and 'receipts/2025-07-29_openai.pdf' in z.namelist()
assert 'reports/2025-07_tase.pdf' in z.namelist()        # reports ride along, no zip-specific code
assert client.get('/m/1999-01/all.zip').status_code == 404
# the link is a page action, not a source file: bottom right — after the
# status table, right-aligned via the .zip-link class, never among the
# statement source links at the top
page = client.get('/m/2025-07').text
assert '/m/2025-07/all.zip' in page
assert page.index('/m/2025-07/all.zip') > page.index('</table>')
assert 'class="zip-link"' in page
frag = client.get('/m/2025-07/expand', headers={'HX-Request': 'true'}).text
assert '/m/2025-07/all.zip' in frag
assert frag.index('/m/2025-07/all.zip') > frag.index('</table>')
assert 'class="zip-link"' in frag
```
